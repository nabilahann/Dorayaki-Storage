<?php
    ...
    // Kalo ditekan tombol delete bakalan dihapus dari database
    if(isset($_POST["delete"])){
        $sql = $db->prepare('DELETE FROM dorayaki WHERE rowid = :id');
        $sql->bindValue(':id', $id);
        $sql->execute();

        echo '<script> alert("Varian Dorayaki '.$row["nama"].' berhasil dihapus."); 
    ...
    //kalo ditekan tombol edit akan dibuka halaman edit
    if(isset($_POST["edit"])){
        $lokasi = "Location: ubah_varian.php?id=".$id;
    ...
    if(isset($_POST["ubah_stok"])){
        $lokasi = "Location: perubahan.php?id=".$id;
        //buka halaman pencarian
        header($lokasi);
        exit();
                ...
                <div class="description-detail">
                    ...
                    <form action="" method="POST">
                        <div class="btn-detail">
                            <button type="submit" name="edit" class="btn-detail-click">EDIT</button>
                            <button type="submit" name="ubah_stok" class="btn-detail-click">UBAH STOK</button>
                            <button type="submit" name="delete" class="btn-detail-click"
                            onclick="return confirm('Apakah Anda yakin ingin menghapus varian dorayaki ini?')">
                            DELETE</button>
                        </div>
                    </form>
